typedef /*@null@*/ Task *MaybeTask;

IndexP get_index(const char *path, /*@null@*/ MaybeTask *task, int force);
void fetch_create_directory(const char *path, Element *node);
/*@dependent@*/ Task *fetch_archive(const char *file, Element *group, Index *index);
